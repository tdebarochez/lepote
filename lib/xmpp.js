...
Client = function (conf) {
  ...
  function resourceBindings () {
	...
	self.write('<presence><show>available</show><status>' + conf.status + '</status></presence>');
      }
...
//     sys.puts('setSecure');
  ...
  xml_parser = new libxml.SaxPushParser(function(cb) {
