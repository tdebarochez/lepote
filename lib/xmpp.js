...
exports.Client = function (conf) {
  ...
  var sasl = recontext(function (attrs) {
    stream_events_manager.removeListener('end|mechanism|null', setMechanism);
  ...
  var resourceBindings = recontext(function () {
    ...
    this.emit('resources.binded');
					...
					// <message from='thomas.barochez@gmail.com/GaimFEE82210' to='lepote@im.apinc.org/LePote' type='chat' id='purplebbb03772'><x xmlns='jabber:x:event'><composing/></x><body>def unix</body><html xmlns='http://jabber.org/protocol/xhtml-im'><body xmlns='http://www.w3.org/1999/xhtml'>def unix</body></html><nos:x value='disabled' xmlns:nos='google:nosave'/><arc:record otr='false' xmlns:arc='http://jabber.org/protocol/archive'/></message>
  var onMessageReceive =  recontext(function (attrs) {
    var onBodyReceived = recontext(function (buffer) {
      stream_events_manager.removeListener('end|body|null', onBodyReceived);
      this.emit('message.receivee', attrs.from, buffer, attrs.to, attrs.type, attrs.id);
    }, this);
    ...
    stream_events_manager.addListener('end|body|null', onBodyReceived);
    stream_events_manager.addListener('end|message|null', onMessageReceived);
  }, this),
...
//     sys.puts('setSecure');
  ...
  var stream_events_manager = (new events.EventEmitter)
    ...
    .addListener('end|mechanism|null', setMechanism)
