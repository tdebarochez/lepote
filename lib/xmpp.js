...
//     sys.puts('setSecure');
  ...
  var sendIq = recontext(function (attrs, content, cb) {
	...
	return;
      ...
      stream_events_manager.removeListener('start|iq|null', onIqResponse);
      var onIqResponseEnding = function (buffer) {
	stream_events_manager.removeListener('end|iq|null', onIqResponseEnding);
	cb(buffer);
      };
      stream_events_manager.addListener('end|iq|null', onIqResponseEnding);
    };
    stream_events_manager.addListener('start|iq|null', onIqResponse);
  }, this);
  sendIq.nb_req = 0;
