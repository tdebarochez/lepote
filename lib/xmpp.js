...
Client = function (conf) {
  ...
  function resourceBindings () {
    ...
    function sessionBindings () {
      ...
      stream_events_manager.once('start|session|' + NS_SESSION, sendPresenceInformation);
    }
    stream_events_manager.once('start|bind|' + NS_BIND, sessionBindings);
    self.emit('ready');
  }
  ...
  function onMessageReceive (attrs) {
    ...
    function onBodyReceived (element) {
      self.emit('message', attrs.from, element.nodeValue, attrs.to, attrs.type, attrs.id);
    }

    function onHtmlBodyReceived (element) {
      self.emit('html.message', attrs.from, element.nodeValue, attrs.to, attrs.type, attrs.id);
    }
  ...
  function onPresenceReceive (attrs) {
    ...
    function presenceReceived (element) {
      self.emit('presence.receive', attrs.from, attrs.to, status, priority, attrs.type, element);
    }

    stream_events_manager.once('end|presence|' + NS_CLIENT, presenceReceived);
    stream_events_manager.once('end|priority|' + NS_CLIENT, priorityReceived);
    stream_events_manager.once('end|status|' + NS_CLIENT, statusReceived);
  }
